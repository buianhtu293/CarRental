...
namespace CarRental.Presentation.Controllers
{
	...
	public class ManageController : Controller
	{
		...
		public async Task<IActionResult> EditProfileAsync(EditExtraProfileModel model)
		{
            ...
            user.UserName = model.UserName; // Update username too
			user.FullName = model.FullName;
            user.Email = model.UserEmail;
            user.PhoneNumber = model.PhoneNumber;
			user.Province = model.ProvinceName;
			user.District = model.DistrictName;
			user.Ward = model.WardName;
			user.LicenseId = model.LicenseId;

			// Handle license image update
			if (model.LicenseImage != null) // only if new file is uploaded
			{
				var result = await _photoService.AddPhotoAsync(model.LicenseImage);
				user.LicenseImage = result.SecureUrl.AbsoluteUri;
			}
			else if (!string.IsNullOrEmpty(model.LicenseImageUrl))
			{
				// Keep the existing image
				user.LicenseImage = model.LicenseImageUrl;
			}
